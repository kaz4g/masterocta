    ...
    pub fn deserialize<'de, D: Deserializer<'de>>(deserializer: D) -> Result<u64, D::Error> {
            ...
            || !text.bytes().all(|byte| byte.is_ascii_digit())
        {
            return Err(serde::de::Error::custom(
                "expected a canonical decimal PCM frame",
            ));
        }
    ...
    pub fn validate(self, total: u64) -> Result<Self, AudioError> {
        if self.start_frame >= self.end_frame || self.end_frame > total {
            return Err(AudioError::InvalidRequest(
                "range must be nonempty and inside the audio",
            ));
        }
        Ok(self)
    }
...
impl WaveformCache {
    pub fn query(
        ...
        let pyramid = match pyramid {
            ...
            None => {
                ...
                let encoded_len = serde_json::to_vec(&pyramid)
                    .map_err(|error| AudioError::CacheUnavailable(error.to_string()))?
                    .len();
        ...
        if let Some(level) = pyramid.levels.iter().find(|level| {
            ...
            return Ok(WaveformWindow {
                ...
                channel_peaks: level
                    .channel_peaks
                    .iter()
                    .map(|peaks| peaks[start..end].to_vec())
                    .collect(),
            });
        }
        ...
        let mut accumulators: Vec<PeakAccumulator> = (0..pyramid.channels)
            .map(|_| PeakAccumulator::new(frames_per_peak))
            .collect();
        let info = decode_frames(
            snapshot.open()?,
            source_path,
            &current,
            |index, frame, _| {
    ...
    pub fn preview_range(
        ...
        let info = decode_frames(
            ...
            |index, frame, rate| {
                ...
                {
                    return Err(AudioError::InvalidRequest(
                        "range preview is limited to 30 seconds and 16 MiB",
                    ));
                }
                if (range.start_frame..range.end_frame).contains(&index) {
                    for sample in frame {
...
fn verified_source(
    ...
    if meta.len() > MAX_SOURCE_BYTES {
        return Err(AudioError::InvalidRequest(
            "waveform input is limited to 2 GiB",
        ));
    }
    if !meta.is_file() || meta.file_type().is_symlink() {
        return Err(AudioError::SourceUnavailable(
            "expected a regular audio file of at most 2 GiB".into(),
        ));
    }
    ...
    if !opened.is_file()
        ...
        || !file.metadata().map_err(source_io)?.is_file()
    {
        return Err(AudioError::SourceChanged);
    }
...
impl Snapshot {
    fn capture(
        ...
        #[cfg(unix)]
        {
            ...
            fs::remove_file(snapshot.path.as_ref().expect("snapshot path exists"))
                .map_err(cache_io)?;
            snapshot.path = None;
        }
        let mut hasher = Sha256::new();
...
fn decode_frames(
    ...
    if expected > MAX_SOURCE_BYTES / channels as u64 {
        return Err(AudioError::UnsupportedFormat);
    }
    let mut index = 0_u64;
    while let Some(packet) = decoded.next_packet()? {
        ...
        if audio.capacity() > 1_048_576 {
            return Err(AudioError::DecodeFailed(
                "decoded packet exceeds the frame limit".into(),
            ));
        }
        ...
        for frame in buffer.samples().chunks_exact(channels) {
            if index >= expected || frame.iter().any(|sample| !sample.is_finite()) {
                return Err(AudioError::DecodeFailed(
                    "invalid PCM data or frame count".into(),
                ));
            }
            let clamped = [
                frame[0].clamp(-1.0, 1.0),
                frame.get(1).copied().unwrap_or(0.0).clamp(-1.0, 1.0),
            ];
...
fn build_pyramid(
    ...
    while levels
        .last()
        .is_some_and(|level| level.channel_peaks[0].len() > 1)
    {
        let previous = levels.last().expect("base level exists");
        levels.push(Level {
            frames_per_peak: previous.frames_per_peak * 4,
            channel_peaks: previous
                .channel_peaks
                .iter()
                .map(|peaks| aggregate_peaks(peaks, 4))
                .collect(),
        });
    }
    Ok(Pyramid {
...
fn load_pyramid(path: &Path, asset_id: &str) -> Result<Option<Pyramid>, AudioError> {
    ...
    if pyramid.version != ANALYZER_VERSION
        || pyramid.asset_id != asset_id
        || pyramid.sample_rate == 0
        || !(1..=2).contains(&pyramid.channels)
        || pyramid.frame_count == 0
        || pyramid.frame_count > MAX_SOURCE_BYTES / u64::from(pyramid.channels)
        || pyramid.levels.is_empty()
        || pyramid.levels.len() > 32
    {
        return Ok(None);
    }
    let mut scale = pyramid.levels[0].frames_per_peak;
    if scale < BASE_FRAMES
    ...
    for (index, level) in pyramid.levels.iter().enumerate() {
        ...
        if level.frames_per_peak != scale
            || level.channel_peaks.len() != usize::from(pyramid.channels)
            || level.channel_peaks.iter().any(|peaks| {
                peaks.len() as u64 != count
                    || peaks.iter().any(|peak| {
                        !peak.min.is_finite()
                            || !peak.max.is_finite()
                            || peak.min < -1.0
        ...
        if index > 0 {
            ...
            for (peaks, prior) in level.channel_peaks.iter().zip(&previous.channel_peaks) {
                if *peaks != aggregate_peaks(prior, 4) {
                    return Ok(None);
                }
            }
        }
    ...
    if pyramid
        .levels
        .last()
        .is_none_or(|level| level.channel_peaks[0].len() != 1)
    {
        return Ok(None);
    }
    Ok(Some(pyramid))
...
fn reserve_cache_space(directory: &Path, target: &Path, needed: u64) -> Result<(), AudioError> {
    if needed > MAX_CACHE_BYTES {
        return Err(AudioError::CacheUnavailable(
            "waveform cache entry is too large".into(),
        ));
    }
...
mod tests {
    ...
    ) -> (TempDir, PathBuf, ContentHash, String) {
        let directory = TempDir::new().unwrap();
        let path = directory.path().join("synthetic.wav");
        let mut pcm = Vec::new();
        for index in 0..frames {
            for channel in 0..usize::from(channels) {
                pcm.extend_from_slice(&sample(index, channel).to_le_bytes());
            }
        }
        fs::write(&path, encode_pcm_wav(&pcm, rate, channels).unwrap()).unwrap();
        let hash = ContentHash::parse(format!(
            "sha256:{:x}",
    ...
    fn query(range: Option<(u64, u64)>, target_points: usize) -> WaveformQuery {
        WaveformQuery {
            range: range.map(|(start_frame, end_frame)| FrameRange {
                start_frame,
                end_frame,
            }),
            target_points,
    ...
    fn frame_wire_format_is_lossless_and_rejects_noncanonical_or_numeric_values() {
        let range = FrameRange {
            start_frame: 9_007_199_254_740_993,
            end_frame: u64::MAX,
        };
        ...
        for invalid in [
            ...
            assert!(serde_json::from_str::<FrameRange>(&format!(
                "{{\"startFrame\":{invalid},\"endFrame\":\"20\"}}"
            ))
            .is_err());
        }
    }
    ...
    fn stereo_polarity_and_silence_are_preserved_without_touching_the_source() {
        let (_fixture, path, hash, asset) =
            fixture(
                8000,
                2,
                65536,
                |_, channel| if channel == 0 { 16384 } else { -16384 },
            );
        ...
        for _ in 0..2 {
            let result = cache
                .query(&asset, &hash, &path, &query(None, 64), || true)
                .unwrap();
            assert_eq!(result.channels, 2);
            assert!(result.channel_peaks[0]
                .iter()
                .all(|peak| peak.min > 0.49 && peak.max > 0.49));
            assert!(result.channel_peaks[1]
                .iter()
                .all(|peak| peak.min < -0.49 && peak.max < -0.49));
    ...
    fn zoom_reads_real_single_frames_and_excludes_attacks_outside_half_open_range() {
        ...
        let cache = WaveformCache::open(directory.path()).unwrap();
        cache
            .query(&asset, &hash, &path, &query(None, 32), || true)
            .unwrap();
        let result = cache
            .query(&asset, &hash, &path, &query(Some((101, 108)), 32), || true)
            .unwrap();
    ...
    fn changed_live_source_is_rejected_even_when_a_pyramid_is_cached() {
        ...
        assert!(matches!(
            cache.query(&asset, &hash, &path, &query(None, 32), || true),
    ...
    fn invalid_and_truncated_requests_never_return_partial_success() {
        ...
        for range in [(0, 0), (2, 1), (1000, 1001)] {
            assert!(cache
                .query(&asset, &hash, &path, &query(Some(range), 32), || true)
                .is_err());
        }
        for points in [0, 31, 4097, usize::MAX] {
            assert!(cache
                .query(&asset, &hash, &path, &query(None, points), || true)
                .is_err());
        }
        let mut bytes = fs::read(&path).unwrap();
    ...
    fn malformed_cache_regenerates_and_superseded_work_stops_without_a_cache_write() {
        ...
        let cache = WaveformCache::open(directory.path()).unwrap();
        assert!(matches!(
            cache.query(&asset, &hash, &path, &query(None, 32), || false),
            Err(AudioError::Cancelled)
        ));
        ...
        let expected = cache
            .query(&asset, &hash, &path, &query(None, 32), || true)
            .unwrap();
        let cache_path = directory.path().join(format!(
            "waveform-v2-{}.json",
            asset.strip_prefix("asset:v1:").unwrap()
        ));
    ...
    fn symlink_cache_and_source_fail_closed() {
        ...
        assert!(cache
            .query(&asset, &hash, &link, &query(None, 32), || true)
            .is_err());
        let cache_path = directory.path().join(format!(
            "waveform-v2-{}.json",
            asset.strip_prefix("asset:v1:").unwrap()
